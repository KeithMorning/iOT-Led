...
async def light_loop():
    ...
    while True:
        ...
        #print('button state is '+str(switch_status))
        LED_status = relay.state()
        #print('relay state is '+str(LED_status))
        if switch_status != switch_status_last:
            if switch_status == 0 and switch_status_last == 1:
                LED_status = 0 if LED_status else 1
            print("button "+str(switch_status)+" relay "+str(relay.state()))
            relay.set_state(LED_status)
            print("open or close led "+str(LED_status))
        ...
        if time_cnt >= 20:
            ...
            if LED_status == 1:
                ...
                on = 1
            mqtt_report(mqtt_client, color, name, on, brightness)
            time_cnt = 0
        time_cnt = time_cnt+1
        time.sleep_ms(50)
